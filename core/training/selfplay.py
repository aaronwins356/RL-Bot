...
class CurriculumStage:
    ...
    opponent_type: (
        str  # "basic_script", "rule_policy", "selfplay", "checkpoint", "fast_opponent"
    )
...
class SelfPlayManager:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.losses = 0
        self.stage_matches = {
            i: {"wins": 0, "losses": 0} for i in range(len(self.stages))
        }

        logger.info(
            f"SelfPlayManager initialized with {len(self.stages)} curriculum stages"
        )
        for stage in self.stages:
            logger.info(
                f"  Stage {stage.stage_id}: {stage.name} ({stage.game_mode}, {stage.opponent_type})"
            )
    ...
    def add_opponent(self, model_path: Path, elo: float = 1500.0, timestep: int = 0):
        ...
        logger.info(
            f"Added opponent to pool: {model_path} (Elo: {elo:.0f}, timestep: {timestep})"
        )
    ...
    def select_opponent(
        ...
        if current_stage.opponent_type == "basic_script":
            ...
            weights = np.array(
                [1.0 / (1.0 + i) for i in range(len(self.opponent_pool))]
            )
            weights /= weights.sum()
