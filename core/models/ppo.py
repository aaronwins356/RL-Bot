...
class PPO:
    ...
    def __init__(
        ...
        self.reward_scale_update_freq = self.config.get(
            "reward_scale_update_freq", 10000
        )
    ...
    def compute_gae(
        ...
        if self.use_dynamic_lambda and explained_variance is not None:
            ...
            current_lambda = self.min_gae_lambda + lambda_factor * (
                self.max_gae_lambda - self.min_gae_lambda
            )
        else:
    ...
    def update_reward_scaling(self, rewards: np.ndarray):
        ...
        if self.running_reward_std > 0:
            self.reward_scale = np.clip(
                1.0 / (self.running_reward_std + 1e-8), 0.1, 10.0
            )
    ...
    def update(
        ...
        with torch.no_grad():
            ...
            stats["explained_variance"] = (
                1 - np.var(y_true - y_pred) / (var_y + 1e-8) if var_y > 0 else 0
            )
